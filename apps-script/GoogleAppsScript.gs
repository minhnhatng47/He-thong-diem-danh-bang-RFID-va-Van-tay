...
const ATTENDANCE_EARLY_MINUTES = 10; // Cho phep diem danh som toi da 10 phut
...
function handleRequest(e) {
  ...
  try {
    ...
    if (action === "set_early_minutes_10") {
      return json(updateEarlyMinutesTo10());
    }
...
function autoUpdateSessions(dotFilter) {
  ...
  for (let i = 0; i < refreshed.length; i++) {
    ...
    const endTime = buildDateTime(row[3], row[5]);
    const earlyMin = getAttendanceEarlyMinutes(row[6]);
...
function setMakeupSession(dot, buoi, ngay, giobd, giokt, ghichu) {
  ...
  const choPhepSom = getAttendanceEarlyMinutes(originalRow[6]);
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem("Cập nhật sớm 10 phút", "updateEarlyMinutesTo10")
    .addSeparator()
...
function getAttendanceEarlyMinutes(value) {
  const minutes = Number(value);

  if (!Number.isFinite(minutes) || minutes <= 0) {
    return ATTENDANCE_EARLY_MINUTES;
  }

  return Math.min(minutes, ATTENDANCE_EARLY_MINUTES);
}

function updateEarlyMinutesTo10() {
  const sh = getSheet(SHEET_LICH);
  ...
  if (lastRow < 2) {
    return {
      ok: true,
      updated: 0,
      message: "LichHoc chua co du lieu"
    };
  }

  const rowCount = lastRow - 1;
  sh.getRange(2, 7, rowCount, 1).setValue(ATTENDANCE_EARLY_MINUTES);

  return {
    ok: true,
    updated: rowCount,
    minutes: ATTENDANCE_EARLY_MINUTES,
    message: "Da cap nhat ChoPhepSom thanh 10 phut"
  };
}
